"""add connected_accounts table and plaid fields to assets and liabilities

Revision ID: 287c26651cd8
Revises: 
Create Date: 2025-12-29 22:00:01.103308+00:00
...
def upgrade() -> None:
    ...
    assets_columns = [col['name'] for col in inspector.get_columns('assets')] if 'assets' in existing_tables else []
    liabilities_columns = [col['name'] for col in inspector.get_columns('liabilities')] if 'liabilities' in existing_tables else []
    
    # Add Plaid fields to assets table (if they don't exist)
    if 'connected_account_id' not in assets_columns:
        op.add_column('assets', sa.Column('connected_account_id', sa.String(length=36), nullable=True))
    if 'is_connected' not in assets_columns:
        op.add_column('assets', sa.Column('is_connected', sa.Boolean(), nullable=False, server_default='false'))
    if 'last_synced_at' not in assets_columns:
        op.add_column('assets', sa.Column('last_synced_at', sa.DateTime(), nullable=True))
    
    # Add Plaid fields to liabilities table (if they don't exist)
    if 'connected_account_id' not in liabilities_columns:
        op.add_column('liabilities', sa.Column('connected_account_id', sa.String(length=36), nullable=True))
    if 'is_connected' not in liabilities_columns:
        op.add_column('liabilities', sa.Column('is_connected', sa.Boolean(), nullable=False, server_default='false'))
    if 'last_synced_at' not in liabilities_columns:
...
def downgrade() -> None:
    ...
    # Remove Plaid fields from liabilities table
    ...
    # Remove Plaid fields from assets table
